...
#[derive(Debug, Clone)]
pub struct OldSpParameters2<D: PredictiveProbabilityFunction + Clone> {
...
impl<D: PredictiveProbabilityFunction + Clone> OldSpParameters2<D> {
    pub fn new(
...
impl<D: PredictiveProbabilityFunction + Clone> FullConditional for OldSpParameters2<D> {
...
impl<D: PredictiveProbabilityFunction + Clone> PartitionSampler for OldSpParameters2<D> {
    fn sample<T: Rng>(&self, rng: &mut T) -> Clustering {
        engine_full(self, None, Some(rng)).0
    }
}

impl<D: PredictiveProbabilityFunction + Clone> ProbabilityMassFunction for OldSpParameters2<D> {
...
impl<D: PredictiveProbabilityFunction + Clone> NormalizedProbabilityMassFunction
    for OldSpParameters2<D>
{
}

impl<D: PredictiveProbabilityFunction + Clone> HasPermutation for OldSpParameters2<D> {
...
impl<D: PredictiveProbabilityFunction + Clone> HasVectorShrinkage for OldSpParameters2<D> {
...
fn engine_full<'a, D: PredictiveProbabilityFunction + Clone, T: Rng>(
    parameters: &'a OldSpParameters2<D>,
    target: Option<&[usize]>,
    rng: Option<&mut T>,
) -> (Clustering, f64) {
...
mod tests {
    ...
    fn test_goodness_of_fit_constructive() {
        ...
        for target in Clustering::iter(n_items) {
            ...
            let parameters =
                OldSpParameters2::new(target, shrinkage, permutation, baseline_distribution)
                    .unwrap();
