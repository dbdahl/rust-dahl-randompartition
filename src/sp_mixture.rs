// Mixture-Based Shrinkage partition distribution
...
impl<D: PredictiveProbabilityFunction + Clone> FullConditional for SpMixtureParameters<D> {
...
impl<D: PredictiveProbabilityFunction + Clone> PartitionSampler for SpMixtureParameters<D> {
    fn sample<T: Rng>(&self, rng: &mut T) -> Clustering {
        engine_full(self, None, Some(rng)).0
    }
}

impl<D: PredictiveProbabilityFunction + Clone> ProbabilityMassFunction for SpMixtureParameters<D> {
...
impl<D: PredictiveProbabilityFunction + Clone> NormalizedProbabilityMassFunction
    for SpMixtureParameters<D>
{
}

impl<D: PredictiveProbabilityFunction + Clone> HasPermutation for SpMixtureParameters<D> {
...
impl<D: PredictiveProbabilityFunction + Clone> HasVectorShrinkageProbabilities
    for SpMixtureParameters<D>
{
...
fn engine_full<D: PredictiveProbabilityFunction + Clone, T: Rng>(
    parameters: &SpMixtureParameters<D>,
    target: Option<&[usize]>,
    rng: Option<&mut T>,
) -> (Clustering, f64) {
    if use_slow_implementation() {
