...
use dahl_salso::optimize::{CMLossComputer, VICMLossComputer};
use std::slice;
...
impl<'a> CPPParameters<'a> {
    pub fn new(center: &'a Partition, rate: Rate, mass: Mass, discount: Discount) -> Option<Self> {
...
pub fn log_pmf(target: &Partition, parameters: &CPPParameters) -> f64 {
    let vi_computer = VICMLossComputer::new(1.0, &parameters.log2cache);
    ...
    let vi = vi_computer.compute_loss(
        &target_as_working,
        &parameters
            .center_as_clusterings
    ...
    crp_log_pmf(target, &crp_parameters) - parameters.rate.unwrap() * vi
}
...
mod tests {
    ...
    fn test_pmf() {
        ...
        for center in Partition::iter(n_items) {
            ...
            let parameters = CPPParameters::new(&center, rate, mass, discount).unwrap();
